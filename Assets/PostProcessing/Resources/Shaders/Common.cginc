...
#define GREY_DOT half3(0.299, 0.587, 0.114)
...
inline float  SafeHDR(float c) { return min(c, HALF_MAX); }
inline float2 SafeHDR(float2 c) { return min(c, HALF_MAX); }
inline float3 SafeHDR(float3 c) { return min(c, HALF_MAX); }
inline float4 SafeHDR(float4 c) { return min(c, HALF_MAX); }
