...
struct VaryingsMultitex
{
    ...
    half4 uv : TEXCOORD0;    // Original UV
    //half2 uv01 : TEXCOORD1;  // Alternative UV (supports v-flip case)
    //half2 uvSPR : TEXCOORD1; // Single pass stereo rendering UV
};
...
VaryingsMultitex VertMultitex(AttributesDefault v)
{
    ...
    o.uv.xy = uvAlt;
    o.uv.zw = UnityStereoTransformScreenSpaceTex(uvAlt);
    //o.uv01 = uvAlt;
...
half3 ReconstructViewPos(half2 uv, half depth, float2 p11_22, float2 p13_31)
{
    return half3((uv * 2.0 - 1.0 - p13_31) / p11_22 * CheckPerspective(depth), depth);
}

// Sample point picker
half3 PickSamplePoint(half2 uv, float index)
{
    ...
    //                                   vvvvvvvvvvvv
    half u = frac(UVRandom(0.0, index + uv.x * 1e-10) + gn) * 2.0 - 1.0;
    half theta = (UVRandom(1.0, index + uv.x * 1e-10) + gn) * UNITY_PI_2;
#else
    half u = UVRandom(uv.x + _Time.x, uv.y + index) * 2.0 - 1.0;
    half theta = UVRandom(-uv.x - _Time.x, uv.y + index) * UNITY_PI_2;
#endif
    half3 v = half3(CosSin(theta) * sqrt(1.0 - u * u), u);
    // Make them distributed between [0, _Radius]
    half l = sqrt((index + 1.0) / _SampleCount) * _Radius;
    return v * l;
}
...
half4 FragAO(VaryingsMultitex i) : SV_Target
{
    ...
    half2 p13_31 = half2(unity_CameraProjection._13, unity_CameraProjection._23);

    // View space normal and depth
    half3 norm_o;
    half depth_o = SampleDepthNormal(UnityStereoScreenSpaceUVAdjust(uv, _CameraDepthTexture_ST), norm_o);
    ...
    for (int s = 0; s < _SampleCount; s++)
    {
        ...
        half2 uv_s1_01 = (spos_s1.xy / CheckPerspective(vpos_s1.z) + 1.0) * 0.5;

        // Depth at the sample point
        half depth_s1 = SampleDepth(UnityStereoScreenSpaceUVAdjust(uv_s1_01, _CameraDepthTexture_ST));

        // Relative position of the sample point
        half3 vpos_s2 = ReconstructViewPos(uv_s1_01, depth_s1, p11_22, p13_31);
...
half4 FragBlur(VaryingsMultitex i) : SV_Target
{
    ...
    fixed4 p0  = tex2D(_MainTex, i.uv.zw);
    fixed4 p1a = tex2D(_MainTex, i.uv.zw - delta * 1.3846153846);
    fixed4 p1b = tex2D(_MainTex, i.uv.zw + delta * 1.3846153846);
    fixed4 p2a = tex2D(_MainTex, i.uv.zw - delta * 3.2307692308);
    fixed4 p2b = tex2D(_MainTex, i.uv.zw + delta * 3.2307692308);

//#if defined(BLUR_SAMPLE_CENTER_NORMAL)
//    fixed3 n0 = SampleNormal(i.uv.zw);
//#else
...
half4 FragComposition(VaryingsMultitex i) : SV_Target
{
    ...
    half ao = BlurSmall(_OcclusionTexture, i.uv.zw, delta);
...
VaryingsMultitex VertCompositionGBuffer(AttributesDefault v)
{
    VaryingsMultitex o;
    o.pos = v.vertex;
#if UNITY_UV_STARTS_AT_TOP
    o.uv.xy = v.texcoord.xy * float2(1.0, -1.0) + float2(0.0, 1.0);
#else
...
CompositionOutput FragCompositionGBuffer(VaryingsMultitex i)
{
