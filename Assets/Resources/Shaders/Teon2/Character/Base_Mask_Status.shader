Shader "Teon2/Character/Base_Mask_Status"
{
    Properties
    {
        ...
        _MaskAreaInten("MaskArea Intensity", Float) = 0
        [HDR]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f i, float face : VFACE) : SV_Target
            {
                ...
                col.rgb = lerp(lerp(col, GreyColor(col), _BlendLuminance), _Rim, _BlendRim);
                col.a = lerp(1, _Rim, _BlendRim);

                col *= (_LightColor0) * (1 - mask);

                float4 maskArea = col * (mask) * _MaskColor * _MaskAreaInten;
