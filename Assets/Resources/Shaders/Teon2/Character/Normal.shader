Shader "Teon2/Character/Normal"
{
    Properties
    {
        ...
        _MRPMap("Metal/Roughess/Illumin Map R:Metallic G:Roughness B:Self-Illumin", 2D) = "white" {}
        _MetallicFixed("Metallic Fixed", Range(0, 1)) = 1
        _RoughessFixed("Roughness Fixed", Range(0, 1)) = 1
        _IlluminFixed("Illumin Fixed", Range(0, 10)) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f i, float facing : VFACE) : SV_Target
            {
                ...
                //// 高度图中描述的高度数据
                //half2 parallaxDelta = GetParallaxDelta(_MRPCol.b, _ParallaxFixed, _WorldPos);
                ////uv偏移
                //i.uv += parallaxDelta;
                ...
                float4 rim = pow(saturate(1 - _NDotV), _Pow) * _Strength * _RimColor;

                float4 ret = 0;
                float atten = 0;
                //
                float3 baseCol = (UNITY_LIGHTMODEL_AMBIENT + _Diffuse + _Specualr + _PointLight + rim);
                float3 naturalCol = baseCol * (1 - _MRPCol.b) * _LightColor0 * (_NDotL);
                float3 darkSide = baseCol * (1 - _MRPCol.b) * _LightColor0 * (1 - _NDotL) * _DarknessFixed;
                float3 illuminCol = baseCol * (_MRPCol.b) * _IlluminFixed;
                #ifdef LIGHTMAP_ON
                    float3 lm = LK_DecodeLightmap(UNITY_SAMPLE_TEX2D(unity_Lightmap, i.lightmapUV));
                    atten = 0;
                    #ifdef SHADOWS_SHADOWMASK
                        float4 rawOcclusionMask = UNITY_SAMPLE_TEX2D(unity_ShadowMask, i.lightmapUV);
                        float realtimeShadowAttenuation = SHADOW_ATTENUATION(i);
                        atten = lerp(realtimeShadowAttenuation, rawOcclusionMask, SHADOWN_ATTEN);
                        //lm *= atten.r * 0.5 + 0.5;
                    #endif
                    ret.rgb = (naturalCol + illuminCol + darkSide) * _Albedo * lm * atten + _IndirectSpecualr;
                #else
                    atten = SHADOW_ATTENUATION(i);
                    ret.rgb = (naturalCol + illuminCol + darkSide) * _Albedo * atten + _IndirectSpecualr;
                #endif
