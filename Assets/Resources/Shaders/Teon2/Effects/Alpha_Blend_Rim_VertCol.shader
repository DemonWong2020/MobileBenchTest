...
Shader "Teon2/Effects/Alpha_Blend_Rim_VertCol" 
{
    Properties 
    {
    ...
    SubShader 
    {
        ...
        Pass 
        {
            ...
            uniform sampler2D _Main_Texture; uniform half4 _Main_Texture_ST;
...
/////// Diffuse:
                half NdotL = max(0.0,dot( normal, lightDirection ));
                half3 directDiffuse = max( 0.0, NdotL) * _LightColor0;
                half3 indirectDiffuse = UNITY_LIGHTMODEL_AMBIENT.rgb; // Ambient Light
                half4 _Main_Texture_var = tex2D(_Main_Texture, i.uv0) * _Main_Color;
                //half3 diffuseColor = (_Main_Color.rgb*_Main_Texture_var.rgb);
                half3 diffuse = (directDiffuse + indirectDiffuse) * _Main_Texture_var;
////// Emissive:
                half3 emissive = (pow(pow(1.0-max(0,dot(normal, viewDirection)), (_Fresnel_Range)),10.0) * (_Fresnel_Itensity * 20.0) * _Fresnel_Color.rgb);
/// Final Color:
                half3 finalColor = diffuse + emissive;
                fixed4 finalRGBA = fixed4(finalColor,(_Main_Color.a*_Main_Texture_var.a*_Fresnel_Color.a)) * i.color;
